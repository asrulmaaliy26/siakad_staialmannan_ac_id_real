...
class Proyek extends BaseController
{
    function __construct()
    {
        ...
        $this->halaman_controller = "proyek";
        $this->halaman_label = "Proyek";
    }
    ...
    public function index()
    {
        ...
        if ($this->request->getMethod(true)=='POST') {
            if($this->request->getVar('aksi')=='hapus' && $this->request->getVar('id')){
                $dt = $this->proyek->find($this->request->getVar('id'));
                if($dt['id_ujian']){ #memastikan ada data
                    
                    $aksi = $this->proyek->delete($this->request->getVar('id'));
                    if($aksi == true){
            ...
            if($this->request->getVar('aksi')=='simpan'){
    		    ...
    		    if(!$this->validate($aturan)){
                    ...
                    if(empty($this->request->getVar('id_proyek'))){
                        $record = [
    ...
    function rincian()
    {
        ...
        $data['templateJudul'] = "Rincian Proyek ".$data['nama_proyek'];
        $data['controller'] = $this->halaman_controller;
        $data['aktif_menu'] = $this->halaman_controller;
        $data['metode']    = 'rincian';
        return view(session()->get('akun_group_folder')."/$this->halaman_controller/".$data['metode'], $data);
