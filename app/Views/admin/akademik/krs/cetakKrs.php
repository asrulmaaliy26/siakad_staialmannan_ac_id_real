    ...
    <div class="boxkartu">
        <div class="header">
            ...
            <div style="font-size: 14pt; font-family: 'Arial', sans-serif;"><strong>STAI AL-MANNAN TULUNGAGUNG</strong></div>
				...
				if (!empty($data['mata_kuliah'])) {
					...
					foreach ($data['mata_kuliah'] as $row): 
						...
						<tr>
							...
							<td><?=(!empty($dtMk['Kd_Dosen']))?getDataRow('data_dosen',['Kode' => $dtMk['Kd_Dosen']])['Nama_Dosen']:''; ?></td>
						</tr>
    ...
    <?php $mk_tl = dataDinamis('data_ljk', ['Status_Nilai !=' => 'L', 'id_his_pdk' => $data['krs']['id_his_pdk'], 't_akad <' => $data['krs']['kode_ta']], 'smt_mhs ASC') ;
            if(!empty($mk_tl)){
    ?>
      ...
      <table width="100%" style=" topntail: 0.02cm solid #000000;" border="1" autosize="1.8">
        ...
        <tbody>
          ...
          <tr >
            ...
            <td><?=(!empty(getDataRow('master_matakuliah', ['kode_mk' => $r->kode_mk_feeder])))?getDataRow('master_matakuliah', ['kode_mk' => $r->kode_mk_feeder])['nama_mk']:$r->kode_mk_feeder;?></td>
            <td align="center"><?=getDataRow('tahun_akademik', ['kode' => $r->t_akad])['tahunAkademik'];?> <?=getDataRow('tahun_akademik', ['kode' => $r->t_akad])['semester'] == '1'?'Gasal':'Genap';?> (<?=$r->smt_mhs;?>)</td>
