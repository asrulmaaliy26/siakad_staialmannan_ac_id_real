            		                ...
            		                <tr>
            		                    <th class="text-center">
                                            <div class="icheck-success d-inline">
                                                <input type="radio" id="H" name="radio1">
                                                <label for="H">H</label>
                                            </div>
                                        </th>
                                        <th class="text-center">
                                            <div class="icheck-primary d-inline">
                                                <input type="radio" id="S" name="radio1">
                                                <label for="S">S</label>
                                            </div>
                                        </th>
                                        <th class="text-center">
                                            <div class="icheck-warning d-inline">
                                                <input type="radio" id="I" name="radio1">
                                                <label for="I">I</label>
                                            </div>
                                        </th>
                                        <th class="text-center">
                                            <div class="icheck-danger d-inline">
                                                <input type="radio" id="A" name="radio1">
                                                <label for="A">A</label>
                                            </div>
                                        </th>
            		                </tr>
...
$(function() {
    ...
    $('#data_mhs > thead > tr > th #H').eq(0).on('click', function() {
        var th_checked = this.checked;

        $(this).closest('table').find('tbody > tr').each(function() {
            var row= this;
            if(th_checked) $(row).addClass(active_class).find('.abs_h').eq(0).prop('checked', true);
            else $(row).removeClass(active_class).find('.abs_h').eq(0).prop('checked', false);
        });
    });

    $('#data_mhs > thead > tr > th #S').eq(0).on('click', function() {
        var th_checked = this.checked;

        $(this).closest('table').find('tbody > tr').each(function() {
            var row= this;
            if(th_checked) $(row).addClass(active_class).find('.abs_s').eq(0).prop('checked', true);
            else $(row).removeClass(active_class).find('.abs_s').eq(0).prop('checked', false);
        });
    });

    $('#data_mhs > thead > tr > th #I').eq(0).on('click', function() {
        var th_checked = this.checked;

        $(this).closest('table').find('tbody > tr').each(function() {
            var row= this;
            if(th_checked) $(row).addClass(active_class).find('.abs_i').eq(0).prop('checked', true);
            else $(row).removeClass(active_class).find('.abs_i').eq(0).prop('checked', false);
        });
    });

    $('#data_mhs > thead > tr > th #A').eq(0).on('click', function() {
        var th_checked = this.checked;

        $(this).closest('table').find('tbody > tr').each(function() {
            var row= this;
            if(th_checked) $(row).addClass(active_class).find('.abs_a').eq(0).prop('checked', true);
            else $(row).removeClass(active_class).find('.abs_a').eq(0).prop('checked', false);
        });
    });
